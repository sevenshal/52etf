...
重构后排名前列的仍有 常熟银行(+408.9%)、杭州银行(+362.8%)、青岛银行(+186.8%)。

根因是第 5.4 节早就写明的模型局限，现在因为非金融股的数字被修正而**更加突出**：
`fair P/B = (ROE − g) / (股权成本 − g)`，银行 beta 低导致股权成本只有 4.4~5.4%，分母被
压到 1.4~2.4 个百分点，算出来的合理市净率是 3~4 倍，而中国银行股长期在 0.6 倍附近交易。

**纯 CAPM + Gordon 框架捕捉不到市场在给资产质量/隐性风险定的价。** 这不是参数问题，是
框架不适用。可选的方向：把金融股单独成表不与非金融混排、或者引入不良率/拨备覆盖率做调整。
在解决之前，**扫描结果里金融股的 return% 不能直接采信**。
