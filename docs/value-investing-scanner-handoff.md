...
| `backend/scripts/survey_mainbz_segment_coverage.py` | 普查 `fina_mainbz` 分部披露颗粒度，决定分部估值(SOTP)值不值得做 |
...
| `backend/tests/test_value_investing_implied.py` | 反推市场隐含假设的往返求解、单调方向、天花板与越界状态测试 |
| `backend/tests/test_value_investing_capex_cycle.py` | 扩产期识别（扩产 vs 失血）与只豁免 FCFF 一条闸门的测试 |
...
## 15. 反推市场隐含假设（2026-09-11）

### 15.1 为什么

正向的 `return%` 只回答"按我们的假设值多少"，而使用者真正要判断的是"市场的假设合不合理"。
把公式倒过来解出**市场隐含的输入值**，再和模型自己估的那个数对照，分歧就落在明面上，
可以拿行业常识去证伪。金融股的隐含 ROE（第 12.7 节）早就是这么做的，这一节把它推广到
全市场，并给金融股补上对称的隐含股权成本。

### 15.2 三个反推量，以及它们各自的天花板

`_market_implied_assumptions()` 对非金融股解三次，每次固定其余输入：

| 反推量 | 固定什么 | 可解区间 | 说明 |
|---|---|---|---|
| 隐含近端增速 | WACC、ROIC | 约 **0.5x ~ 1.1x** 公允价值 | 受 `g ≤ ROIC×0.9` 再投资约束 |
| 隐含 ROIC | WACC、增速 | 约 **~1.3x** 公允价值 | 再投资率降到 0 就到头了 |
| 隐含贴现率 | 增速、ROIC | **很宽**（下界=永续增长+150bp） | 任何价位都解得出 |

**这张表是这一节最重要的结论**：增速和 ROIC 的反推都有天花板，所以真正贵的股票——也就是
最想问"市场在假设什么"的那一批——只有**隐含贴现率**给得出数字。这不是求解器不行，是模型在说
"没有任何可行的经营假设能解释这个价格"，那句话本身就是答案；而隐含贴现率把它翻译成一句
能直接判断的话：*按这套现金流假设，以当前价买入的年化回报率只有百分之几*。

实测（ROIC 18%、近端增速 10%、WACC 9% 的样板公司）：顶格增速 16.2% 只能把价值抬到公允的
1.12 倍，ROIC 拉到 100% 也只到 1.28 倍，而贴现率降到 6% 就有 2.11 倍。

### 15.3 求解器：方向由端点决定，越界报状态而不是夹到端点

`_solve_monotone()` 是个二分求解器，**不写死单调方向**，由两个端点自己决定。这不是过度设计：
本模型里价值对增长率的单调方向取决于 ROIC、再投资率上限、以及 `min(g, ROIC×0.9)` 这个夹子
在哪里生效，实测既有增也有减，而且和教科书"ROIC<WACC 时增长毁灭价值"的直觉未必一致——
近端增速降下来的同时 NOPAT 基数本身也在缩，两个效应方向相反。

目标落在区间外时返回 `status`（`above_range`/`below_range`）而不是夹到端点冒充一个解。
前端据此显示"再高的增速也解释不了当前价格"，而不是一个 `-`。

### 15.4 输出字段

非金融：`implied_near_term_growth_pct` / `implied_roic_pct` / `implied_discount_rate_pct`，
各自带 `_status` 和与模型采用值的 `_gap_pct`。金融：`implied_cost_of_equity_pct`
（与既有的 `implied_sustainable_roe_pct` 是同一公式的两种解法——一个固定 r 解 ROE，
一个固定 ROE 解 r；两个一起看才能分辨市场是不信这个 ROE，还是要了更高的风险补偿）。

个股详情页把隐含贴现率提到了头条指标行，估值明细里三项齐全。

### 15.5 仍未做

- 隐含增速/ROIC 的天花板是模型的再投资约束带来的，不是求解区间不够大。想让贵的股票
  也能解出经营假设，得改的是 `MAX_REINVESTMENT_RATE` 或显式期长度，那是方法论改动，
  需要单独评估。
- 没有在生产库上统计过全市场三项反推各自的可解比例。

---

## 16. 扩产期通道（2026-09-11）

### 16.1 依据：闸门在为同一笔增长罚两次

`min_fcf_positive_years=3` 曾经是拦截扩产期科技公司的主要闸门之一。但回头看模型内部，
这一条其实和 DCF 自相矛盾：

- DCF（`_two_stage_reinvestment_value`）用 **`再投资率 = g/ROIC`** 给增长计价，
  **并不减实际资本开支**；
- 而质量闸门用**实际** FCFF（`经营现金流 − 资本开支 + 税后利息`）的为正年数去淘汰。

于是一家正在铺产能的公司被罚了两次：DCF 里已经为增长付过再投资的代价，闸门又因为
这笔支出把它直接判出局。第 15 节那套反推能给出「市场在假设什么」，但前提是这家公司
得先活着走到估值这一步。

### 16.2 扩产 vs 失血：三个条件同时成立

`_capex_cycle_assessment()`：

| 条件 | 阈值 | 为什么 |
|---|---|---|
| 资本开支 / 折旧摊销 | ≥ **1.5** | 支出不是维持现有资产，是在铺新产能 |
| 经营现金流为正的年数 | ≥ **3**（近5年） | **分水岭**：经营现金流都是负的，那不是扩产是烧钱 |
| 营收复合增速 | ≥ **0%** | 否则是在给一门衰退的生意加杠杆 |

### 16.3 这是「并一条通道」，不是「放宽闸门」

满足三条的公司，**只有**「FCFF 为正年数不足」这一条从淘汰理由降级成提示（notes），
并写清降级的依据（capex/D&A 倍数、经营现金流为正年数）。其余闸门一条都不放过：
ROIC-WACC 价差、经营现金流/净利润、资产负债率、内在价值同比。有测试逐条验证——
把这四项任意一项打到不合格，扩产期公司照样被淘汰。

不满足三条的公司，FCFF 为负仍然是淘汰理由，行为完全不变。

### 16.4 输出与展示

新增 `in_capex_cycle` / `capex_to_daa` / `ocf_positive_years`（通过与未通过两条路径都带）。
个股详情页挂「扩产期」金色标签并在指标区列出两个判据；扫描页表格在股票名后挂「扩产」小标签。

### 16.5 仍未做

- **基准 NOPAT 没有针对扩产期做任何调整**。新产能还没贡献利润，近3年 NOPAT 均值会低估
  这类公司的正常盈利能力，DCF 的起点因此偏低。要修得引入「达产后正常化盈利」的假设，
  那是方法论改动（且很容易变成给高估值找借口），需要单独评估，这次没动。
- 阈值 1.5 / 3年 / 0% 是拍的，没有用全市场数据校准过；上线后应该看一眼被这条通道
  救回来的是哪些股票，是否符合直觉。

---

## 17. 分部估值(SOTP)的可行性：先普查再决定（2026-09-11）

结论先行：**tushare 只能支撑"相对估值型"分部估值，而且只对少数披露够细的公司成立。**

`fina_mainbz`(主营业务构成，2000积分；全市场版 `fina_mainbz_vip` 需 5000积分)是 tushare
唯一的分部数据接口，字段只有 `bz_item`/`bz_sales`/`bz_cost`/`bz_profit`(通常是毛利口径)：

| 能做 | 做不了 |
|---|---|
| 分部收入、毛利、毛利率、增速 → EV/Sales、EV/毛利 分部加总 | 分部净利、分部资产、分部资本开支 → **分部 DCF / 分部 PE / 分部 ROIC 一个都算不出来** |

更关键的是披露颗粒度。已知实例：长电科技(600584.SH)2025年报「主营业务分产品」**只有一行**
（芯片封测 388.71亿、毛利率 13.95%），`fina_mainbz` 对它输出的就是这一行。真正能拆的是
年报「主要控股参股公司分析」那张子公司表——**tushare 没有任何接口提供它**，只能解析
cninfo 的 PDF。

所以没有直接建同步管道，而是先写了只读诊断脚本
`backend/scripts/survey_mainbz_segment_coverage.py`：抽样跑全市场，统计每家的分部条目数和
最大单项收入占比，判据是「条目数 ≥3 且最大项 <70%」。**这个脚本必须在有 tushare token 的
环境里跑（生产环境），本地跑不了**，所以覆盖度的答案目前是空的。

判读标准写在脚本输出里：可做 SOTP 的比例低于两三成的话，建同步管道不划算，更值得做的是
对少数控股型/多元化公司单独处理。
