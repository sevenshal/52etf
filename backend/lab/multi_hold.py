...
if __name__ == "__main__":
    ...
    r = run_multi(pairs, 45.0, fear_map, bars_map, trading_days)
    print(f"多持仓轮动(统一log-z 1.25 + 贪即卖): {r['total']:.2f}% 回撤 {r['mdd']:.2f}% 夏普 {r['sharpe']:.2f} "
          f"空仓 {r['idle_ratio']:.1f}% 买卖 {len(r['buys'])}/{len(r['sells'])}")
    print("基准 B(单持仓最恐慌): 212.47% / -9.97% / 2.13 / 空仓 64.5% / 10买10卖")
    print("\n明细:")
    for s in r["sells"]:
        print(f"  卖 {s['date']} {s['symbol']} @{s['price']:.3f} pnl {round(s['pnl']):,}")
    for b in r["buys"]:
        print(f"  买 {b['date']} {b['symbol']} @{b['price']:.3f}")
