...
from ..core.database import get_db_ctx, SzdtTradeStock, TradingLog, TradingState, StockCooldown
...
class USAutoTrader:
    ...
    def _fetch_candidates(self) -> List[Dict]:
        ...
        with get_db_ctx() as db:
            rows = db.query(SzdtTradeStock).filter(
                SzdtTradeStock.account_id == self.account_id,
    ...
    def _get_next_stock(self) -> Dict:
        """按状态索引 + 冷却筛选，每次只取一只。"""
        with get_db_ctx() as db:
            state = db.query(TradingState).filter_by(account_id=self.account_id, cli_id=self.cli_id).first()
            if not state:
                state = TradingState(account_id=self.account_id, cli_id=self.cli_id, current_index=0)
                db.add(state)
            ...
            db.query(StockCooldown).filter(
                StockCooldown.account_id == self.account_id,
                StockCooldown.cli_id == self.cli_id, 
