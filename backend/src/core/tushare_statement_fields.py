"""tushare 财务报表/财务指标接口的完整字段清单（单一事实来源）。

三个消费方共用这一份定义，避免"接口拉回来了、却在写库前被列白名单静默丢弃"这类
只能靠肉眼比对三处列表才能发现的问题：

- `core/services/tushare.py`：拼 ``fields=`` 请求参数、做数值列类型转换
- `core/analytics_database.py`：生成 DuckDB 表结构、给存量表补列
- `robot/a_stock_base_data_sync.py`：归一化与批量落库的列白名单

字段取的是 tushare 官方文档「输出参数」表的**全集**，包含当前没有任何代码用到的
字段。这是有意为之：tushare 的增量同步只在"该股票完全没有记录"或"最新公告已过期"
时才会重新抓取，为新增列补齐历史必须走 ``force_full_refresh=True`` 的一次性全量
回填（还要手工把脚本部署到生产机），代价远高于一次性多存几十个 FLOAT 列。

各接口都有部分字段的「默认显示」是 N（例如 fina_indicator 的 ``ocf_to_or``、
``roe_avg``），不显式传 ``fields`` 就拿不到，所以这里必须列全而不能省略参数。
"""
from __future__ import annotations

from typing import Sequence

# 所有报表共用的主键列：``ts_code`` + ``end_date`` 一定要取，且不参与下面的分组。
STATEMENT_KEY_FIELDS = ("ts_code", "end_date")


def build_api_fields(*field_groups: Sequence[str]) -> str:
    """拼出 tushare ``fields=`` 参数（主键列 + 各分组列，保持顺序、去重）。"""
    names: list[str] = list(STATEMENT_KEY_FIELDS)
    seen = set(names)
...
# --- 利润表(tushare `income`，官方文档 https://tushare.pro/document/2?doc_id=33) ---
INCOME_DATE_FIELDS = (
    "ann_date",                       # 公告日期
    "f_ann_date",                     # 实际公告日期
)
...
# --- 资产负债表(tushare `balancesheet`，官方文档 https://tushare.pro/document/2?doc_id=36) ---
...
# --- 现金流量表(tushare `cashflow`，官方文档 https://tushare.pro/document/2?doc_id=44) ---
...
# --- 财务指标(tushare `fina_indicator`，官方文档 https://tushare.pro/document/2?doc_id=79) ---
...
INCOME_API_FIELDS = build_api_fields(INCOME_DATE_FIELDS, INCOME_TEXT_FIELDS, INCOME_NUMERIC_FIELDS)
BALANCESHEET_API_FIELDS = build_api_fields(
    BALANCESHEET_DATE_FIELDS, BALANCESHEET_TEXT_FIELDS, BALANCESHEET_NUMERIC_FIELDS
)
CASHFLOW_API_FIELDS = build_api_fields(
