...
def _load_xueqiu_board_momentum(
    ...
    # For intraday board pricing, compare each held constituent's live snapshot
    # price with its close on the 5-day weight comparison date.
    _attach_xueqiu_today_ratio(connection, stock_rows, compare_snapshot_date)
    ...
    connection.execute(
        """
        CREATE OR REPLACE TEMP TABLE xueqiu_stock_intraday_prices (
            stock_symbol VARCHAR,
    ...
    if intraday_price_rows:
        connection.executemany(
            "INSERT INTO xueqiu_stock_intraday_prices VALUES (?, ?)",
            intraday_price_rows,
        )
    ...
    for source in BOARD_SOURCES:
        ...
        result[source] = _aggregate_board_rows(
            ...
            min_stocks=XUEQIU_BOARD_MIN_STOCKS,
            stock_directions_table="xueqiu_stock_directions",
            stock_intraday_prices_table="xueqiu_stock_intraday_prices",
            direction_fn=_xueqiu_direction,
        )
    return result
...
def load_xueqiu_board_holding_symbols(
    ...
    """Return Xueqiu-held stocks for one board (同花顺板块或申万一/二/三级行业) at the requested snapshot."""
    ...
    try:
        if not all(
            ...
            for table in (XUEQIU_TOP_HOLDINGS_SNAPSHOT_TABLE, membership_table)
        ):
            return {
...
def load_xueqiu_board_history(
    ...
    connection = _connect_duckdb()
    try:
        required = (XUEQIU_TOP_HOLDINGS_SNAPSHOT_TABLE, membership_table, daily_table)
        ...
        return {
            ...
            "contrarian_boards": {
                source: top_contrarian_boards(
                    items, min_stocks=XUEQIU_CONTRARIAN_BOARD_MIN_STOCKS
                )
