...
def _eastmoney_5d_ratio_fields(
    current_weight: Any,
    previous_weight: Any,
    current_price: Any,
    previous_price: Any,
    ...
    两列共用此函数，保证两处的取整口径、锚点语义一致（与雪球侧同规则）：
    ...
    - ``direction_5d`` 由上述三个取整后的值判定（见 ``_eastmoney_direction``）。
    """
    ...
    return {
        ...
        "direction_5d": _eastmoney_direction(weight_multiple, momentum_multiple, ratio),
    }
...
def _attach_eastmoney_5d_momentum(
    ...
    *,
    ...
    与「权重和排名历史」表的 5日权价比完全同源（统一走 ``_eastmoney_5d_ratio_fields``）：
    ...
    for item in items:
        ...
        item.setdefault(weight_multiple_key, None)
    ...
    for item in items:
        ...
        fields = _eastmoney_5d_ratio_fields(
            item.get("composite_weight_pct"),
            item.get("weight_5d_ago"),
            current_price,
            previous_price,
        )
        item[weight_multiple_key] = fields["weight_multiple_5d"]
        item[momentum_multiple_key] = fields["momentum_multiple_5d"]
        item[ratio_key] = fields["weight_price_ratio_5d"]
...
def _attach_eastmoney_history_5d_ratios(
    ...
    且比值改由 ``_eastmoney_5d_ratio_fields`` 计算，与最新综合持仓列同一套取整口径。
    传入 ``global_snapshot_dates`` 时用全局序列定位锚点；缺省时退回本股票行序兜底。
    """
    lookback = EASTMONEY_TOP_HOLDINGS_RANK_COMPARE_TRADING_DAYS
    today = _china_today()
    for row in rows:
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Unable to load global snapshot dates for 东方财富持仓 %s: %s",
                normalized_symbol,
                exc,
            )
        _attach_eastmoney_history_5d_ratios(rows, global_snapshot_dates)
        return {
            "available": True,
            "active_only": active_only,
            "symbol": normalized_symbol,
