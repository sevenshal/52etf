...
from ...robot.a_stock_innovation100 import compute_yearly_returns, load_benchmark_index_curves
from ...robot.a_stock_micro400 import (
    DEFAULT_START_DATE,
    INDEX_CODE,
    AStockMicro400Builder,
    load_a_stock_micro400_summary,
    rebuild_a_stock_micro400,
)
from .account import valid_account


router = APIRouter(prefix="/api/a-stock-micro400", tags=["A Stock Micro 400"])
...
class AStockMicro400RebuildRequest(BaseModel):
...
def _update_job(task_id: str, **kwargs):
    ...
    publish_event(account_id, "a_stock_micro400_job", event_payload)
...
def _run_rebuild_job(task_id: str, start_date: date, end_date: Optional[date]):
    ...
    try:
        result = rebuild_a_stock_micro400(
            db,
        ...
        db.rollback()
        logger.exception("A stock micro400 rebuild failed")
        _update_job(
            task_id,
            status="failed",
            message=str(exc),
...
def _removal_codes(rows: Iterable[AStockMicro400Rebalance]) -> set:
    codes = set()
    for row in rows:
...
def _load_constituent_name_map(
    db: Session,
    rows: Iterable[AStockMicro400Rebalance],
) -> Dict[str, str]:
    codes = _removal_codes(rows)
    if not codes:
        return {}
    constituents = (
        db.query(AStockMicro400Constituent)
        .filter(
            AStockMicro400Constituent.index_code == INDEX_CODE,
            AStockMicro400Constituent.ts_code.in_(codes),
        )
        .order_by(AStockMicro400Constituent.rebalance_date.desc())
        .all()
    )
...
def _serialize_constituent(row: AStockMicro400Constituent) -> Dict:
    return {
        "index_code": row.index_code,
...
@router.post("/rebuild")
def start_rebuild(
    payload: AStockMicro400RebuildRequest,
    ...
    thread = threading.Thread(
        ...
        daemon=True,
        name=f"a-stock-micro400-{task_id[:8]}",
    )
    thread.start()
...
def get_detail(
    ...
    summary = load_a_stock_micro400_summary(db)
    levels = (
        db.query(AStockMicro400Level)
        .filter(AStockMicro400Level.index_code == INDEX_CODE)
        .order_by(AStockMicro400Level.date.asc())
        .all()
    )
    rebalances = (
        db.query(AStockMicro400Rebalance)
        .filter(AStockMicro400Rebalance.index_code == INDEX_CODE)
        .order_by(AStockMicro400Rebalance.rebalance_date.desc(), AStockMicro400Rebalance.id.desc())
    ...
    if rebalance_id:
        selected_rebalance = db.query(AStockMicro400Rebalance).filter(
            AStockMicro400Rebalance.index_code == INDEX_CODE,
            AStockMicro400Rebalance.id == rebalance_id,
        ).first()
    ...
    if selected_rebalance:
        selected_constituents = (
            ...
            .filter(
                AStockMicro400Constituent.index_code == INDEX_CODE,
                AStockMicro400Constituent.rebalance_id == selected_rebalance.id,
            )
...
def get_rebalance_detail(
    ...
    rebalance = db.query(AStockMicro400Rebalance).filter(
        AStockMicro400Rebalance.index_code == INDEX_CODE,
        AStockMicro400Rebalance.id == rebalance_id,
    ).first()
    if not rebalance:
        raise HTTPException(status_code=404, detail="未找到该调样记录")
    constituent_name_map = _load_constituent_name_map(db, [rebalance])
    constituents = (
        db.query(AStockMicro400Constituent)
        .filter(
            AStockMicro400Constituent.index_code == INDEX_CODE,
            AStockMicro400Constituent.rebalance_id == rebalance.id,
        )
        .order_by(AStockMicro400Constituent.rank.asc())
        .all()
    )
...
def rebuild_a_stock_micro400_for_scheduler(
    start_date: Optional[date] = None,
    end_date: Optional[date] = None,
    full_rebuild: bool = False,
) -> Dict:
    with get_db_ctx() as db:
        builder = AStockMicro400Builder(db)
        try:
