...
from ...core.database import get_db_session, TradingState, StockCooldown, SzdtTradeStock, TradingLog
...
async def get_trade_opportunities(
    ...
    with get_db_session(account_id) as db:
        # 获取或创建交易状态
        state = db.query(TradingState).filter_by(cli_id=cli_id).first()
        if not state:
