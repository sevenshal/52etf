...
# 第 1 天极恐放量买入；第 3 天起贪婪（第 4 天贪恐 90）；估值点位第 5 天起才落到 10（极度高估）
FEAR = [50.0, 20.0, 50.0, 80.0, 90.0, 80.0, 80.0, 80.0, 50.0, 50.0]
VALUATION = [50.0, 50.0, 50.0, 50.0, 50.0, 10.0, 10.0, 10.0, 10.0, 10.0]
...
def test_sell_gate_holds_until_valuation_is_expensive():
    ...
    gated = _run_backtest(frame, _params(valuation_sell_max=20), 1_000_000.0, detailed=True)

    assert _trade_days(plain, "SELL") == [_day(3)]
    assert _trade_days(gated, "SELL") == [_day(5)]
    assert "估值点位 10.0" in [t for t in gated["trades"] if t["action"] == "SELL"][0]["reason"]


def test_force_sell_greed_overrides_valuation_gate():
    frame = _frame(FEAR, VALUATION)
    result = _run_backtest(
        frame, _params(valuation_sell_max=20, valuation_force_sell_greed=85), 1_000_000.0, detailed=True,
    )
    assert _trade_days(result, "SELL") == [_day(4)]


def test_buy_gate_blocks_expensive_buy_but_missing_valuation_passes():
    blocked = _run_backtest(_frame(FEAR, VALUATION), _params(valuation_buy_min=80), 1_000_000.0, detailed=True)
    assert _trade_days(blocked, "BUY") == []

    no_valuation = [np.nan] * len(FEAR)
    passed = _run_backtest(_frame(FEAR, no_valuation), _params(valuation_buy_min=80), 1_000_000.0, detailed=True)
    assert _trade_days(passed, "BUY") == [_day(1)]


def test_valuation_gate_uses_signal_day_value_with_next_open_execution():
    # 次日开盘成交：第 5 天（估值 10）是信号日，第 6 天开盘成交
    frame = _frame(FEAR, VALUATION)
    result = _run_backtest(
        frame, _params(valuation_sell_max=20, execute_next_open=True), 1_000_000.0, detailed=True,
    )
...
def test_seesaw_applies_gate_to_each_leg_with_its_own_valuation():
    ...
    params = _params(
        ...
        swap_threshold=45, valuation_sell_max=20,
    )
...
def test_search_counts_valuation_candidates():
    ...
    widened = SOXLFearSearchParams(valuation_sell_max_values=[None, 20, 30], valuation_window_values=[252, 504])
