...
class SoxlFearBacktestVolumeSignalTest(TestCase):
    ...
    def test_slippage_pessimistic_buys_high_sells_low(self):
        """滑点=-1 最悲观：买入按当日最高价、卖出按当日最低价。"""
        dates = pd.bdate_range("2024-01-01", periods=3).date
        base_df = pd.DataFrame({
        ...
        params = SOXLFearStrategyParams(
            ...
            rebalance_threshold_pct=0.0, slippage_pct=-1.0,
        )
        result = _run_backtest(base_df, params, 10000.0, detailed=True)
        ...
        self.assertEqual(1, len(buys))
        self.assertEqual(1, len(sells))
        # 悲观买入 = 当日最高价 110，悲观卖出 = 当日最低价 91
        self.assertAlmostEqual(110.0, buys[0]["price"])
        self.assertAlmostEqual(91.0, sells[0]["price"])

    def test_slippage_percent_and_stamp_duty(self):
        """滑点>=0 按成交价滑点；印花税卖出收取。"""
        dates = pd.bdate_range("2024-01-01", periods=3).date
        base_df = pd.DataFrame({
            "date": dates, "open": [100.0, 101.0, 102.0], "high": [110.0, 111.0, 112.0],
        ...
        buys = [t for t in result["trades"] if t["action"] == "BUY"]
        sells = [t for t in result["trades"] if t["action"] == "SELL"]
        # 买入价 = 成交价×(1+0.1%)=100.1；卖出价 = 101×(1-0.1%)=100.899
        self.assertAlmostEqual(100.1, buys[0]["price"])
        self.assertAlmostEqual(100.899, sells[0]["price"], places=3)
        # 印花税 0.05% 按卖出金额收取
