...
def initialize(context):
    # 全局变量初始化
    g.api_base_url = "https://api.framework.cn/api/trade"
...
def handle_data(context, data):
    ...
    try:
        ...
        response = requests.post(
            ...
            headers=g.headers,
            json={
                "cli_id": 'Backtest' if backtest else 'Prod',
                "backtest": backtest,
                "orders": [
                    {
        ...
        log.info("获取交易机会成功 %s" % result["msg"])
        ...
        for opp in opportunities:
            ...
            # 兜底检查当日是否已有订单
            today_orders = get_all_orders(symbol)
            if today_orders:
                log_to_backend("INFO", "%s 当日已存在订单，跳过操作" % (opp['name']))
            # 获取限价
