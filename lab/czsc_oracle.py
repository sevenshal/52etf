"""CZSC 1.0.1 comparison oracle — research only, no production caller.

This is the former ``analyze_bars_czsc_legacy`` adapter, moved out of the
backend so production code no longer imports the compiled ``czsc`` signal
engine.  Use it to diff the native structure engine against CZSC's official
分型 / 笔 / 中枢 and its ``cxt_*`` buy/sell signal functions.

``analyze_bars_czsc_legacy`` returns the same plain-dict shape the native
``analyze_bars`` does, so a caller can swap the two for a controlled
comparison.
"""
...
def analyze_bars_czsc_legacy(
    ...
    *,
    ...
    """CZSC 1.0.1 adapter, kept for controlled comparison experiments."""
