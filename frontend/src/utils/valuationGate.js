// 估值点位闸门的选项与展示（情绪量能回测页、A股情绪量能实盘页共用）
...
export const formatValuationGate = (params) => {
  ...
  const parts = [];
  if (params.valuation_buy_min != null) {
    parts.push(`买≥${params.valuation_buy_min}`);
  }
  if (params.valuation_sell_max != null) {
    parts.push(`卖≤${params.valuation_sell_max}`);
