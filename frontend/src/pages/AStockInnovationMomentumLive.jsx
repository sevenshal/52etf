...
const AStockInnovationMomentumLive = () => {
  ...
  const loadConfigs = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await request.get('/api/a-stock-innovation-momentum-live/configs');
      setConfigs(data || []);
  ...
  const loadDetail = useCallback(async (configId) => {
    ...
    try {
      const { data } = await request.get(`/api/a-stock-innovation-momentum-live/configs/${configId}/detail`);
      setDetail(data);
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      const { data } = selectedConfig?.id
        ? await request.put(`/api/a-stock-innovation-momentum-live/configs/${selectedConfig.id}`, payload)
        : await request.post('/api/a-stock-innovation-momentum-live/configs', payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      await request.delete(`/api/a-stock-innovation-momentum-live/configs/${selectedConfig.id}`);
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    try {
      await request.post(`/api/a-stock-innovation-momentum-live/configs/${selectedConfig.id}/sync`, null, { timeout: 600000 });
  ...
  const tradeColumns = [
    ...
    { title: '原因', dataIndex: 'reason_detail', width: 280, ellipsis: true },
  ];

  const eventColumns = [
    { title: '日期', dataIndex: 'date', width: 110 },
    { title: '类型', dataIndex: 'direction', width: 80, render: value => <Tag color="blue">{value}</Tag> },
    { title: '股票', dataIndex: 'symbol', width: 110 },
    { title: '名称', dataIndex: ['payload', 'name'], width: 100 },
    { title: '行业', dataIndex: ['payload', 'industry'], width: 110 },
    { title: '排名', dataIndex: ['payload', 'rank'], width: 80, align: 'right' },
    ...
    { title: '混合涨跌', dataIndex: ['payload', 'window_return_pct'], width: 100, align: 'right', render: formatPercent },
  ];
  ...
  return (
    <div style={{ padding: 24 }}>
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }}>
        <Col>
          <Title level={4} style={{ margin: 0 }}>A股创新100动量虚拟盘</Title>
        </Col>
        <Col>
          <Space>
        ...
        <Col xs={24} lg={7}>
                    ...
                    <List.Item.Meta
                      ...
                      description={
                        <Space direction="vertical" size={2}>
                          ...
                          <Text type="secondary">超额：{formatPercent(item.runtime?.excess_return)}</Text>
                        </Space>
                      }
                    />
                  </List.Item>
          ...
          <Card title={selectedConfig?.id ? '参数' : '新增参数'}>
            <Form form={form} layout="vertical" initialValues={DEFAULT_FORM_VALUES}>
              ...
              <Row gutter={12}>
                <Col span={12}>
                  <Form.Item name="auto_sync_time" label="自动同步时间" rules={[{ required: true }]}>
                    <Input />
                  </Form.Item>
                </Col>
                <Col span={12}>
              ...
              <>
                <Card style={{ marginBottom: 16 }}>
                  <Descriptions column={{ xs: 1, sm: 2, xl: 4 }} size="small">
                    ...
                    <Descriptions.Item label="策略收益">{formatPercent(metrics.total_return ?? summary.total_return)}</Descriptions.Item>
                    <Descriptions.Item label="创新100基准">{formatPercent(metrics.benchmark_total_return)}</Descriptions.Item>
                    <Descriptions.Item label="超额收益">{formatPercent(metrics.excess_return)}</Descriptions.Item>
                    ...
                    <Descriptions.Item label="待执行信号">{metrics.pending_signal_date || '-'}</Descriptions.Item>
                  </Descriptions>
                </Card>
                    ...
                    items={[
                      {
                        key: 'holdings',
                        label: '持仓',
                        children: <Table rowKey="symbol" size="small" columns={holdingColumns} dataSource={detail?.holdings || []} pagination={false} scroll={{ x: 920 }} />,
                      },
                      {
                        key: 'trades',
                        label: '交易',
                      ...
                      },
                      {
                        key: 'yearly',
                        label: '分年',
                        children: <Table rowKey="year" size="small" columns={yearlyColumns} dataSource={yearlyStats} pagination={false} scroll={{ x: 760 }} />,
                      },
                      {
                        key: 'events',
                        label: '排名',
...
export default AStockInnovationMomentumLive;
