...
const EastmoneyHoldingsResearch = () => {
  ...
  const boardColumns = useMemo(() => [
    ...
    {
      title: (
        <Tooltip title="用该板块内当前被东方财富组合持有且有时点价的成分股计算：逐股以最新时点价对比5日窗口首日收盘价，再等权合成。">
          板块涨跌
        </Tooltip>
      ),
  ...
  const historyColumns = useMemo(() => [
    ...
    {
      title: <Tooltip title="相对上一条持仓交易日：综合权重倍数 ÷ 股价倍数。最新盘中日期使用实时日K。">权价比</Tooltip>,
      dataIndex: 'weight_price_ratio_today',
      width: 100,
      align: 'right',
      sorter: (a, b) => Number(a.weight_price_ratio_today || 0) - Number(b.weight_price_ratio_today || 0),
      render: value => (value == null ? '-' : `${Number(value).toFixed(2)}x`),
    },
    {
      title: '方向',
      dataIndex: 'direction_today',
      width: 106,
      filters: EASTMONEY_DIRECTIONS.map(direction => ({ text: direction, value: direction })),
      onFilter: (value, record) => record.direction_today === value,
      render: value => (value ? <Tag color={EASTMONEY_DIRECTION_COLORS[value] || 'default'}>{value}</Tag> : '-'),
    },
